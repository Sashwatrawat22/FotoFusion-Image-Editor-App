...
# Flask utils
from flask import Flask,flash, request, render_template,send_from_directory, Blueprint
...
# Set the configuration options using app.config
app.config['CARTOON_FOLDER'] = 'cartoon_images'
app.config['UPLOAD_FOLDER'] = 'uploads'


@second.route('/uploads/<filename>')
def upload_img(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)

@second.route('/cartoon_images/<filename>')
...
def predict():
    if request.method == 'POST':
        ...
        img = cv2.imread(file_path)
         ...
         # Process the image based on the selected style
        if style =="Style1":
